<?php
/**
 * Claude (Anthropic) text provider for Inkbridge Generator.
 *
...
/**
 * Class Inkbridge_Gen_Claude
 *
 * Implements the Inkbridge_Gen_Text_Provider interface using the Anthropic Messages API.
 */
class Inkbridge_Gen_Claude implements Inkbridge_Gen_Text_Provider {
	...
	public function generate( string $system_prompt, string $user_prompt, int $max_tokens = 4096 ): array {
		...
		if ( is_wp_error( $response ) ) {
			throw new RuntimeException( 'Claude API request failed: ' . $response->get_error_message() );
		}
		...
		if ( $status_code !== 200 ) {
			...
			throw new RuntimeException( 'Claude API error (' . $status_code . '): ' . $error_message );
		}

		if ( ! isset( $body['content'][0]['text'] ) ) {
			throw new RuntimeException( 'Claude API returned an unexpected response format.' );
		}
